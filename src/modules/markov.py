        ...
        if ret == 'body':  # Генерируем середину
            if last_words in self.__dictionary:
                ...
                if self.__window == 1:
                    ...
                    return Markov(self.__window-1).subgenerate(last_words, 'body')
            ...
            if last_words in self.__end_dict:
                ...
                if self.__window == 1:
                    ...
                    last_words = ' '.join(
                        last_words.split()[-self.__window+1:])
                    return Markov(self.__window-1).subgenerate(last_words, 'end')

    def generate(self, length: int = None, query: str = None, window: int = None) -> str:
        ...
        def generate_gibberish(length: int) -> str:
            ...
            for _ in range(length):
                ...
                words = Markov(self.__window-1).subgenerate(' '.join(last_words.split(
